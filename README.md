...
0. Update the version number in `version.rb`.
0. Run `rake release`, which will create a git tag for the version, push git commits and tags, and push the `.gem` file to [rubygems.org](https://rubygems.org).
0. Run `rake changelog`, which will auto-generate a changelog of the PRs merged/Issues resolved in the newest release.
